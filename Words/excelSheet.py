...
target_sheets = ['UI', 'API', 'Database', 'Others']

# Create an Excel writer to save results
output_file_path = 'CategorySortedWords.xlsx'
with pd.ExcelWriter(output_file_path, engine='openpyxl') as writer:
    ...
    for sheet_name in target_sheets:
        try:
            ...
            df_sorted.to_excel(writer, sheet_name=f'{sheet_name}_KeywordCounts', index=False)

        except Exception as e:
            print(f"An error occurred while processing sheet '{sheet_name}': {e}")

print(f"Keyword counts have been saved to {output_file_path}")
